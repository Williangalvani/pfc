...
Outra possível interpretação é que a diferença seja devido ao overhead no NOMAD. Todas suas iterações envolvem o disparo de novos processos python, além da escrita e leitura de diferentes arquivos para a comunicação entre os processos.


\begin{table}[]
\centering
\caption{Tempo de execução e numero de iterações por algorítmo}
\label{my-label}
\begin{tabular}{lllll}
                    & Tempo & Iterações &  &  \\
OrthoMADS           & 525   & 206       &  &  \\
